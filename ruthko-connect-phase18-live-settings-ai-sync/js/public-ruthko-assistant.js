(function () {
  ...
  const quickAnswers = [
    ...
    { keys: ['contact', 'email', 'phone', 'call', 'reach'], answer: '__DYNAMIC_CONTACT__', link: 'intake.html', label: 'Open contact form' }
  ];
  ...
  function localAnswer(question) {
    ...
    if (match) {
      if (match.answer === '__DYNAMIC_CONTACT__') {
        const settings = typeof window.ruthkoGetLocalSiteSettings === 'function' ? window.ruthkoGetLocalSiteSettings() : {};
        return { ...match, answer: `You may contact Ruthko at ${settings.contactEmail || 'info@ruthkojobs.com'}, ${settings.contactPhone1 || '+1-701-260-3908'}, or ${settings.contactPhone2 || '+1-240-486-5002'}.` };
      }
      return match;
    }
    return {
